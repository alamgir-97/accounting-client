      ...
      <section className="w-full max-[640px]:h-full flex gap-4 max-[640px]:flex-col max-[640px]:mx-0 justify-center bg-white mb-2 aspect-[675/887]">
        <div className="max-[640px]:w-full max-[640px]:px-0 sm:w-[45vw] md:w-[35vw] lg:w-[26vw] flex flex-col justify-end  border rounded-lg bg-white text-center">
          ...
          <h1
            ...
            className="font-bold mb-2 text-4xl flex flex-col justify-center"
          >
        ...
        <div className="max-[640px]:w-full max-[640px]:px-0 sm:w-[45vw] md:w-[35vw] lg:w-[26vw] flex justify-start border rounded-lg border-gray-50 bg-white">
          {isOpen && (
            <div className="max-[640px]:text-xs sm:text-[10px] md:text-[10px] lg:text-sm p-2 overflow-y-clip">
              <ul className="list-disc pl-4 space-y-1">
                ...
                <li className="">
                  নোট, প্রশ্ন বা উত্তরের উপরের ডাবল ক্লিক করলে তোমার লেখাটা
                  পড়িয়ে নিতেও পারবে।
                </li>
                <li className="">
                  এই ওয়েবসাইটের উদ্দেশ্য তোমাদের পড়ালেখাকে আধুনিক মান উপযোগী,
                  সহজ ও আকর্ষণীয় করে তোলা। পাঠক্রমকে শিখন ঘাটতি না থাকা উপযোগী
                  করা।
                </li>
                <li className="">
                  এটা তোমাদের পড়ালেখা এগিয়ে নেওয়ার সাথে সাথে কম্পিউটার ও
                  অন্যান্য ডিজিটাল ডিভাইসের উপরও দক্ষতা গড়ে তুলতে সাহায্য করবে।
                </li>
              </ul>
            </div>
          )}
...
{/*  sticky max-[640px]:top-[5vh] sm:top-[9vw] md:top-[6vw] z-40  */}
        {chapters.map((chapter, index) => (
          ...
          <div
            key={index}
            className={`w-full px-4 flex flex-col broder rounded-md max-[640px]:h-[84vh] sm:h-[90vh] max-[640px]:overflow-y-scroll max-[640px]:mb-2 max-[640px]:shadow-md max-[640px]:${colors2[index][1]} ${colors2[index][0]} border ${colors2[index][2]}`}
          >
            {/* =================Chapter Heading================= */}

            <h1
              className={`max-[640px]:text-sm md:text-md font-bold text-center w-full py-4 my-1 flex justify-center items-center 
            ...
            <section className="flex justify-between items-center gap-4 border rounded-md p-8" id={index} >
              {/* Left panel: Book */}
              <div className="w-1/2">
                <div>
                  <span className="max-[640px]:pr-1 sm:pr-1 md:pr-2">অধ্যায়: </span>
              <span className="text-white max-[640px]:pr-2 sm:pr-4 md:pr-4">
                {chapter}
              </span>
              </div>
              </div>
              <div className="w-1/2 flex justify-end items-center gap-">
              
              <div className="w-[8vh] h-[8vh] border rounded-md flex items-center justify-center">
                <DraggableModal
                  topic={`পাঠ্য বই`}
            ...
            {/* <section className="flex max-[640px]:flex-col" id={index}>
              ...
              <div className="w-full mt-4">
                <NotesModal noteKey={`/banglaNote_ch${chapter}`}>
                  {chapter}
                </NotesModal>
              </div>
              <div className="w-full mt-4">
                <ShortQuestionsModal
                  questionKey={`/banglaShortQuestions_ch${chapter}`}
                  answerKey={`/banglaShortQuestions_ch${chapter}`}
                >
                  {chapter}
                </ShortQuestionsModal>
              </div>
              <div className="w-full mt-4">
                <BroadQuestionsModal
                  questionKey={`/banglaBroadQuestions_ch${chapter}`}
                  answerKey={`/banglaBroadQuestions_ch${chapter}`}
                >
                  {chapter}
                </BroadQuestionsModal>
              </div>
